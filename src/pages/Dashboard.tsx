...
type SortMethod = 'dateDesc' | 'dateAsc' | 'az' | 'za';
...
export default function Dashboard({ isDemo, user }: Props) {
  ...
  const [hoveredCardSet, setHoveredCardSet] = useState<CardSet | null>(null);
  const [sortOpen, setSortOpen] = useState(false);
  const [sortBy, setSortBy] = useState<SortMethod>('dateDesc');
  ...
  // --- 1. USE QUERY: Fetching Data ---
  const { data: cardSetArray = [], isLoading } = useQuery({
    queryKey: ['cardSets', user?.id],
    queryFn: async () => {
      if (isDemo) {
        const localSets = localStorage.getItem("cardSets");
  ...
  // --- 2. USE MUTATION: Deleting Data ---
  const deleteSetMutation = useMutation({
    mutationFn: async (cardSet: CardSet) => {
      if (isDemo) {
        const localSets = localStorage.getItem("cardSets");
        if (localSets) {
          const parsed: CardSet[] = JSON.parse(localSets);
    ...
    onSuccess: () => {
      // Magically triggers a background refetch so the UI stays in sync
      queryClient.invalidateQueries({ queryKey: ['cardSets', user?.id] });
    },
    onError: (error) => {
      console.error(error);
      alert("Error deleting card set. Please try again");
  ...
  const handleSetDelete = (cardSet: CardSet) => {
    if (!confirm(`Are you sure you want to delete ${cardSet.title}?`)) return;
    deleteSetMutation.mutate(cardSet);
  };

  // --- 3. DERIVED STATE: Sorting Data safely ---
  const sortedCardSets = useMemo(() => {
    const copy = [...cardSetArray];
    switch (sortBy) {
      case 'dateDesc':
  ...
  if (isLoading) return <div className="h-screen w-screen flex items-center justify-center bg-[#88B1CA] text-white">Loading...</div>;

  return (
    <div className="h-screen overflow-y-hidden">
      <div className="h-full w-screen flex flex-col items-center justify-center bg-[#88B1CA]">
        <div className="min-h-[95%] lg:max-w-[75%] md:max-w-[95%] w-full">
          <h1 className="lg:text-3xl md:text-3xl text-xl font-semibold text-white mx-3 mb-4">Welcome{user ? `, ${trimmed}` : ''}</h1>
          <div className='lg:h-[75%] md:h-[75%] h-[80%] w-full flex lg:flex-row md:flex-row flex-col items-center lg:justify-between md:justify-between gap-1'>
            <div className="lg:h-full md:h-full h-[80%] lg:w-[80%] md:w-[80%] w-[95%] bg-white rounded-lg shadow-xl px-4 py-2 overflow-y-scroll flex flex-col items-end gap-1"> 
              <button
                className="flex relative w-[100px] hover:bg-slate-200 bg-slate-100 text-xs px-3 py-1 text-left shadow-md rounded-sm justify-between z-3 cursor-pointer" 
                onClick={() => setSortOpen(!sortOpen)}
              >
                Sort by {sortOpen ? <span>▲</span> : <span>▼</span>}
                {sortOpen && (
                  <div className="absolute left-0 mt-6 w-full bg-slate-100 shadow-md z-6 rounded-sm">
                    <div className="px-3 py-2 hover:bg-[#88B1CA] cursor-pointer" onClick={() => setSortBy('dateDesc')}>Date Newest</div>
                    <div className="px-3 py-2 hover:bg-[#88B1CA] cursor-pointer" onClick={() => setSortBy('dateAsc')}>Date Oldest</div>
                    <div className="px-3 py-2 hover:bg-[#88B1CA] cursor-pointer" onClick={() => setSortBy('az')}>Title A-Z</div>
                    <div className="px-3 py-2 hover:bg-[#88B1CA] cursor-pointer" onClick={() => setSortBy('za')}>Title Z-A</div>
                  </div>
                )}
              </button>
              ...
              <div className='w-full lg:grid md:grid hidden lg:grid-cols-4 md:grid-cols-3 gap-4'>
                <div className='flex flex-col items-center justify-between bg-white border-3 border-transparent hover:border-[#88B1CA] rounded-md p-4 aspect-1/1 cursor-pointer'>
                  <div className='flex justify-center items-center h-7/10' onClick={() => navigate('/createnewset')}>
                    <img src={createImg} alt="Create" className="w-[70%] shadow-md" />
                  </div>
                  <span className="text-sm text-center">Create New Set</span>
                </div>
                
                {sortedCardSets.map((cardSet, i) => (
                  <div
                    key={cardSet.id}
                    className={`flex flex-col items-center justify-between border-3 border-transparent hover:border-[#88B1CA] rounded-md p-4 aspect-1/1 cursor-pointer relative ${i%2 ? 'bg-white':'bg-slate-100'}`}
                    onClick={() => navigate(`/flashcards/${cardSet.id}`)}
                    onMouseEnter={() => setHoveredCardSet(cardSet)}
                    onMouseLeave={() => setHoveredCardSet(null)}
                  >
                    <div className="h-7/10 flex justify-center items-center ">
                      <img src={cardsImg} alt="Cards" className="w-[80%]" />
                    </div>
                    <p className="text-sm font-medium w-full text-center line-clamp-2">{cardSet.title}</p>
                    <button
                      className={`absolute top-0 right-0 p-1 aspect-1/1 flex justify-center items-center z-4 hover:text-red-500 cursor-pointer ${hoveredCardSet === cardSet ? 'opacity-100' : 'opacity-0'}`}
              ...
              {/* Mobile List */}
              <div className='w-full lg:hidden md:hidden grid grid-cols-1 gap-2 my-3'>
                <div className='flex w-full border-transparent bg-[#004D7C] rounded-md cursor-pointer justify-center items-center text-white p-2' onClick={() => navigate('/createnewset')}>
                  <span className="text-xs">Create New Set</span>
                </div>
                {sortedCardSets.map((cardSet, i) => (
                  <div
