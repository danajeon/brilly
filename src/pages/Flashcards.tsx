...
export default function Flashcards({ isDemo }: { isDemo: boolean }) {
    ...
    // --- 1. FETCH CARD SET TITLE ---
    const { data: title = '' } = useQuery({
        queryKey: ['cardSetTitle', cardSetId],
        queryFn: async () => {
            if (isDemo) {
                const storedSets = JSON.parse(localStorage.getItem('cardSets') || '[]');
                const foundSet = storedSets.find((s: any) => s.id === cardSetId);
                return foundSet ? foundSet.title : '';
            }
    ...
    // --- 2. FETCH CARDS ---
    const { data: originalCardsArray = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            if (isDemo) {
                ...
                return storedCards
                    .filter((c: any) => c.cardSet === cardSetId)
                    .sort((a: any, b: any) => a.index - b.index) as Card[];
            }
            const { data, error } = await supabase
                .from("cards")
                .select("*")
    ...
    // --- 3. MUTATION: SAVE ELABORATION ---
    const saveElaborationMutation = useMutation({
        mutationFn: async ({ cardId, elaborationText }: { cardId: string, elaborationText: string }) => {
            if (isDemo) {
                const storedCards = JSON.parse(localStorage.getItem('cards') || '[]');
                const updated = storedCards.map((card: any) =>
                    card.id === cardId ? { ...card, ai: elaborationText } : card
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['cards', cardSetId] });
        },
        onError: (error) => {
    ...
    const handleSaveElaboration = (cardId: string, elaborationText: string) => {
        saveElaborationMutation.mutate({ cardId, elaborationText });
    };

    // --- DERIVED STATE: Shuffling ---
    const displayCards = useMemo(() => {
        if (!isShuffled) return originalCardsArray;
        return [...originalCardsArray].sort(() => Math.random() - 0.5);
    }, [originalCardsArray, isShuffled]);

    const currentCard = displayCards[cardIndex];
    const disableBackButton = cardIndex === 0;
    const disableForwardButton = cardIndex >= displayCards.length - 1;

    useEffect(() => {
        setShowFront(true);
    }, [cardIndex]);

    const addOne = () => setCardIndex(prev => prev + 1);
    const minusOne = () => setCardIndex(prev => prev - 1);
    ...
    if (isLoading) return <div className="h-screen flex items-center justify-center bg-[#88B1CA] text-white">Loading cards...</div>;

    return (
        <div className={`lg:h-screen h-full overflow-y-hidden bg-[#88B1CA] ${displayCards.length > 0 ? 'opacity-100' : 'opacity-0'}`}>
            <div className="lg:h-[95%] w-screen flex flex-col items-center justify-center">
                <div className="flex lg:flex-row flex-col lg:h-full lg:max-w-[75%] w-full lg:items-start items-center lg:gap-8 md:gap-3 gap-0 lg:mt-8 lg:mb-0 my-4">
                    {/* left side */}
                    <div className='lg:h-[80%] lg:w-1/2 md:w-[65%] w-[95%]'>
                        ...
                        <div className='flex justify-center items-center'>
                            <div className="w-[100%] aspect-5/3 bg-white rounded-md shadow-lg/50">
                                {currentCard && (showFront ?
                                    <div
                                        className="w-full h-full flex justify-center items-center text-center lg:text-3xl md:text-3xl text-lg p-4 cursor-pointer"
                                        onClick={() => setShowFront(false)}>
                                        <span>{currentCard.front}</span>
                                    </div>
                                    :
                                    <div
                                        className="w-full h-full flex justify-center items-center text-center lg:text-2xl md:text-lg text-sm p-4 cursor-pointer"
                                        onClick={() => setShowFront(true)}>
                        ...
                        <div className='flex justify-evenly items-center lg:p-2 md:p-4 p-3'>
                            ...
                            <span className='font-semibold text-[#004D7C] lg:text-md text-sm font-bold'>
                                {displayCards.length > 0 ? cardIndex + 1 : 0}/{displayCards.length}
                            </span>
                            <button
                                className={`text-[#004D7C] active:scale-90 ${disableForwardButton ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}
                                onClick={addOne}
                        ...
                        <div className='w-full lg:aspect-5/3 rounded-md lg:pb-0 pb-5'>
                            {currentCard && (
                                <Elaborator
                                    flashcardContent={currentCard.front}
                        ...
                        <div className='h-[90%] overflow-y-scroll pr-3'>
                            <div className='flex flex-col gap-1 overflow-scroll'>
                                {displayCards.length === 0 && (
                                    <p className="text-gray-200">No cards found.</p>
                                )}
                                {displayCards.map((card, i) => (
                                    <div
                                        ...
                                        <div className="lg:w-[30%] w-[25%] aspect-5/3 bg-white rounded-md flex justify-center items-center p-1">
                                            <p className="lg:text-sm text-xs font-medium text-center line-clamp-3">{card.front}</p>
                                        </div>
                                        <div className="w-[65%] flex justify-start items-center">
                                            <p className="lg:text-sm text-xs text-black line-clamp-2">{card.back}</p>
                                        </div>
