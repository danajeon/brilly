...
export default function CreateNewSet({ isDemo, user }: Props) {
  ...
  // --- MUTATION: Create Card Set ---
  const createSetMutation = useMutation({
    mutationFn: async () => {
      let createdCardSetId = `cdst${crypto.randomUUID()}`;
      ...
      if (isDemo) {
        ...
        const { data: cardSetData, error: cardSetError } = await supabase
          .from("cardSets")
          .insert([{ title, id: createdCardSetId, quantity: cards.length, user: user.id }])
          .select()
          .single();
    ...
    onSuccess: () => {
      ...
      // Invalidate dashboard cache so the new deck appears instantly
            ...
            <div className="w-full flex flex-col items-center lg:px-4 md:px-1 overflow-scroll">
              {cards.map((card, index) => {
                ...
                return (
                  <div
                    key={card._cid}
                    className={`flex items-center justify-between w-full transition-shadow ${isDragOver ? "border-t-2" : ""} ${isDragging ? "opacity-60" : "opacity-100"}`}>
                    <p className="lg:flex md:flex hidden w-[3%] justify-start items-center lg:text-lg md:text-lg text-sm font-semibold text-[#004D7C]">{index + 1}</p>
                    <div className="flex flex-1 bg-[#88B1CA] rounded-lg shadow-md my-1">
                      <div
                        draggable
                        ...
                        onDrop={(e) => handleDrop(e, index)}
                        onDragEnd={handleDragEnd}
                        className="text-sm flex lg:flex-row md:flex-row flex-col items-center justify-evenly text-zinc-300 lg:p-1 md:p-1 cursor-grab active:cursor-grabbing">
                        <p className="lg:hidden md:hidden flex justify-start items-center lg:text-lg md:text-lg text-xs font-semibold text-[#004D7C]">{index + 1}</p>
                        <DensityMediumIcon className="lg:scale-100 md:scale-80 scale-60 hover:text-white" />
                        <button onClick={() => deleteCard(index)} className="lg:hidden md:hidden block cursor-pointer z-1">
                          <DeleteForeverIcon className="lg:scale-120 md:scale-100 scale-80 text-black hover:text-red-500" />
                        </button>
                      </div>
                      <div className="flex lg:flex-row md:flex-row flex-col justify-between w-[90%] lg:gap-3 md:gap-3 gap-1 my-1">
                        <div className="flex-1 bg-white rounded-lg lg:p-2 md:p-2 p-1">
                    ...
                    <button onClick={() => deleteCard(index)} className="lg:block md:block hidden cursor-pointer z-1 ml-2">
                      <DeleteForeverIcon className="lg:scale-120 md:scale-100 scale-80 hover:text-red-500" />
                    </button>
                  </div>
                )
              })}
